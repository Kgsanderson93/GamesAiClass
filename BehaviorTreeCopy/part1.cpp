...
int main()
{
    ...
    character *curr = new character(new Kinematic(windowx, windowy, 1, "boid.png"));
    ...
    bool start = false;
    std::cout<<"Time, Eating, beenHit, Dead, Dancing, Pposx, Pposy, lastAct,Pvelx, Pvely, Prot, Mposx, Mposy, Mvelx, Mvely, Mrot, numFruit, PMdis, Meat, mDance\n";
    ...
    while (window.isOpen())
    {
        ...
        if (curr->charmove->dead && mob->movement->dancing <= 0)
        {
            ...
            curr = new character(new Kinematic(windowx, windowy, 1, "boid.png"));
