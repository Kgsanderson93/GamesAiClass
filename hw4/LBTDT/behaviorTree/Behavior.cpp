...
void printStats(BehaviorTree *behaviorTree, std::string name, Status status)
{
   ...
   //////printf("%d\n", i++); // 2
    ...
    {
        /**std::cout << behaviorTree->player->_position.x << "," << behaviorTree->player->_position.y << "," << behaviorTree->mob->_position.x
                  << "," << behaviorTree->mob->_position.y << ","<< distancelength(behaviorTree->player->_position, behaviorTree->mob->_position)
                  <<"," << behaviorTree->player->_velocity.x << ","<< behaviorTree->player->_velocity.y << "," << behaviorTree->mob->_velocity.x 
                  << "," << behaviorTree->mob->_velocity.y << "," << pathlenP << "," << pathlenFm  << "," << name << "\n";*/
    }
}
...
void BehaviorTree::tick(float time)
{
    //////std::cout<<"in tick\n";
    this->time = time;
    //////std::cout<<"root"<<root->strName<<"\n";
    root->tick();
}

Status Behavior::tick()
{
    //////std::cout<<"in tick" << this->strName<<" "<<this->m_eStatus << "\n";
    if (m_eStatus != BH_RUNNING)
    {
        onInitialize();
    }
    //////std::cout<<"in tick 2" << this->strName<<" "<<this->m_eStatus << "\n";

    m_eStatus = update();
    //////std::cout<<"in tick 3" << this->strName<<" "<<this->m_eStatus << "\n";

    if (m_eStatus != BH_RUNNING)
    {
        onTerminate(m_eStatus);
    }
    //////std::cout<<"in tick 4" << this->strName<<" "<<this->m_eStatus << "\n";
...
Status PlayerVisCondition::update()
{
    ...
    if (!path->returnedpath.empty() && path->returnedpath.size() < 15)
    {
        Status status = BH_SUCCESS;
        float distofruitM;
        float distofruitP;
...
Status PlayerHitAction::update()
{
    ...
    if (inRadiusDecel(d))
    {
            ...
            Status status = BH_SUCCESS;
            //printf("This is cheating but I hit\n");
            printStats(behaviorTree, this->strName, status);
            printStats(behaviorTree, this->strName, status);
            printStats(behaviorTree, this->strName, status);
...
Status FruitVisCondition::update()
{
    ...
    while (!behaviorTree->game->goals.empty())
    {
        ...
        if (!p->returnedpath.empty() && p->returnedpath.size() < 15)
        {
            Status status = BH_SUCCESS;
            float distofruitM;
            float distofruitP;
...
Status MoveToFruitAction::update()
{
    ...
    //std::cout<<"fruit "<<behaviorTree->game->GameId<<"\n";
    Path p2 = Path();
    gameTile *goal = behaviorTree->game->getTile(behaviorTree->player->_position);
    ...
    if(this->getStatus()==BH_INVALID|| path->returnedpath.empty()){
...
{
    behaviorTree->game->updategoals(behaviorTree->mob->_position, 0);
    if (inRadiusDecel(distancelength(behaviorTree->game->goals.top().second->coords, behaviorTree->mob->_position)))
    {
...
Status MoveToPlayerAction::update()
{
    ...
    //////std::cout<<behaviorTree->player->_position.x<<" "<< behaviorTree->player->_position.y<<" "<<behaviorTree->mob->_position.x<<" "<<behaviorTree->mob->_position.y<<"\n ";
    if (inRadiusDecel(distancelength(behaviorTree->player->_position, behaviorTree->mob->_position)))
    {
        behaviorTree->player->dead = true;
        behaviorTree->player->isVisible = false;
    ...
    //////std::cout << "Move to player action path len: " << path->returnedpath.size() << " index " << path->idx << " Is satisfied at that index? " << behaviorTree->mob->_satisfied << " \n";
        ...
        if (inRadiusSat(distancelength(behaviorTree->mob->_position, behaviorTree->player->_position)))
        {
            status = BH_SUCCESS;
            printStats(behaviorTree, this->strName, status);
        }
...
void GameId::onInitialize(){
    reset();
}
Status GameId::update() {
            //////std::cout<<this->strName<<"\n";

    if(behaviorTree->game->GameId<div){
        return falsebranch->tick();
    }else{
        return truebranch->tick();
    }
...
Status Peat::update() {
    ...
    bool eat=behaviorTree->player->isEating;
    std::string eats="0";
    if(eat==true){
        eats.assign("1");
    }
    if(div.compare(eats)!=0){
        return falsebranch->tick();
    }else{
        return truebranch->tick();
    }
}
void Fruits::onInitialize(){
    reset();
}
Status Fruits::update() {
            //////std::cout<<this->strName<<"\n";

    int f=atoi(div.c_str());
    if(behaviorTree->game->fruitList.empty() && f!=0){
        return falsebranch->tick();
    }
    if(f<behaviorTree->game->fruitList.size()){
                return falsebranch->tick();
    }else{
        return truebranch->tick();
    }
}
void PDance::onInitialize(){
    reset();
}
Status PDance::update() {
            //////std::cout<<this->strName<<"\n";

    if(behaviorTree->player->dancing <div){
        return falsebranch->tick();
    }
...
void Meat::onInitialize(){
    reset();
}
Status Meat::update() {
            //////std::cout<<this->strName<<"\n";

    bool eat=behaviorTree->mob->isEating;
    std::string eats="0";
    if(eat==true){
        eats.assign("1");
    }
...
void TimeSelector::onInitialize(){
    reset();
}
Status TimeSelector::update() {
            //////std::cout<<this->strName<<"\n";

    if(behaviorTree->time <div){
        return falsebranch->tick();
    }
...
void MDance::onInitialize(){
    reset();
}
Status MDance::update() {
            //////std::cout<<this->strName<<"\n";

    
    if(behaviorTree->mob->dancing <div){
        return falsebranch->tick();
    }
...
void conLearnedSelector::onInitialize(){
    reset();
    
}
void discLearnedSelector::onInitialize(){
    reset();
}
void fg2id::onInitialize(){
   reset();
}
Status fg2id::update() {
            //////std::cout<<this->strName<<"\n";
...
Status ActionBehavior::update(){
    return BH_INVALID;
}
void ActionBehavior::onTerminate(Status){
        
}


void fgidid::onInitialize(){
   reset();
}
Status fgidid::update() {
            //////std::cout<<this->strName<<"\n";
...
Status mgid::update() {
  ...
  /**std::cout<<behaviorTree->game->fruitList.size()<<"\n";
       ...
       //////printf("%d\n", i++);
        ...
        fg = behaviorTree->game->getTile(behaviorTree->mob->_position);
       //////printf("%d\n", i++);
...
void pathlenFp::onInitialize(){
   reset();
}
Status pathlenFp::update() {
    //////std::cout<<this->strName<<"\n";
    int i = 0;
   //////printf("%d\n", i++); // 0
    int pathlenP = 0;
...
void pathlenP::onInitialize(){
   reset();
}
Status pathlenP::update() {
    //////std::cout<<this->strName<<"\n";
    int i = 0;
   //////printf("%d\n", i++); // 0
    int pathlenP = 0;
...
void Mvely::onInitialize(){
   reset();
}
Status Mvely::update() {
            //////std::cout<<this->strName<<"\n";

    if(behaviorTree->mob->_velocity.y<div){
...
void Mvelx::onInitialize(){
   reset();
}
Status Mvelx::update() {
            //////std::cout<<this->strName<<"\n";

if(behaviorTree->mob->_velocity.x<div){
...
void Pvely::onInitialize(){
   reset();
}
Status Pvely::update() {
            //////std::cout<<this->strName<<"\n";

if(behaviorTree->player->_velocity.y<div){
...
void Pvelx::onInitialize(){
   reset();
}
Status Pvelx::update() {
            //////std::cout<<this->strName<<"\n";

    if(behaviorTree->player->_velocity.x<div){
...
void PDistF::onInitialize(){
   reset();
}
Status MDistF::update() {
            //////std::cout<<this->strName<<"\n";
...
void MDistF::onInitialize(){
   reset();
}
Status DmtoP::update() {
            //////std::cout<<this->strName<<"\n";

    if(distancelength(behaviorTree->mob->_position, behaviorTree->player->_position)<div){
...
void pgid::onInitialize(){
   reset();
}
Status Mosy::update() {
        //////std::cout<<this->strName<<"\n";

    if(behaviorTree->mob->_position.y<div){
...
void Mosy::onInitialize(){
   reset();
}
Status Mposx::update() {
        //////std::cout<<this->strName<<"\n";

if(behaviorTree->mob->_position.x<div){
...
void Mposx::onInitialize(){
   reset();
}
Status Pposy::update() {
        //////std::cout<<this->strName<<"\n";

if(behaviorTree->player->_position.y<div){
...
void Pposx::onInitialize(){
   reset();
}


Status LearnedSelector::Update ()
{
...
BehaviorTree::BehaviorTree(){
    //hope youre gonna add that..
}

Status conLearnedSelector::update(){
    return BH_INVALID;

}

Status discLearnedSelector::update(){
    return BH_INVALID;
}
void pathlenFm::onInitialize(){
   reset();
}
Status pathlenFm::update(){
    
    //////std::cout<<this->strName<<" in pathlenFm"<<"\n";
    int i = 0;
    ////printf("%d\n", i++); // 0
    int pathlenP = 0;
    int pathlenFm = 0;
    ...
    //////std::cout<<behaviorTree->game->GameId<<"\n";
    //////std::cout<<behaviorTree->mob->_position.x<<" "<<behaviorTree->mob->_position.y<<"\n";
    gameTile *mg = behaviorTree->game->getTile(behaviorTree->mob->_position);
   ////printf("%d\n", i++); // 1
