...
SteeringData Flocking::calculateAcceleration(Kinematic *cur, flock *flock){
    ...
    if (num_neighbors>0){
        ...
        //temp=sf::Vector2f((pos_avg.x-cur->_position.x), (pos_avg.y-cur->_position.y));
        //temp=normalize(temp);
        //temp*=centering;
        //vals.linear+=temp;

         //temp=sf::Vector2f((vel_avg.x-cur->_velocity.x), (vel_avg.y-cur->_velocity.y));
        //temp=normalize(temp);
        //temp*=matching;
        //vals.linear+=temp;

        
        //vals.linear+=cur->_velocity;
